...
Markdown is rendered through a sanitized viewer (not a raw executable document): source is fetched as `text/plain`, parsed as GFM, and passed through DOMPurify before highlight / KaTeX / Mermaid. Each Mermaid diagram has Code / Split / Preview controls (default Preview), with Preview-only zoom and an in-viewer fullscreen overlay. The Markdown viewer also builds an on-page outline from `h1`–`h3` (right side on wide viewports) with click-to-scroll and scroll spy. That still is not a security boundary for untrusted files. Only preview Markdown from trusted workspaces.
