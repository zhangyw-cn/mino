...
**Scope:** Close Quick Open from the preview iframe, make empty-query recents look like VS Code’s file list, tighten overlay density  
**Approach:** Transparent full-viewport dismiss layer + iframe `Escape` forwarding; remove recents footer; smaller rows  
...
3. **Density:** Result rows are 36px tall with roomy padding, so the overlay feels heavier than VS Code’s Quick Input.
...
4. Tighten overlay input and row spacing toward VS Code Quick Input. Do not add Command Center chevron, `file results` labels, split-editor actions, or Copilot chrome.
...
File rows stay: type chip, basename (match marks `#4fc1ff`), parent path on the right (`margin-left: auto`, match marks `#cccccc`). No `file results` text, no split icon, no Command Center `▾`.

**Density (exact values):**

| Element | From | To |
|---------|------|----|
| `.quick-open-item` `min-height` | 36px | 24px |
| `.quick-open-item` padding | 8px 14px | 4px 12px |
| `#quick-open-input` padding | 10px 14px | 6px 12px |
| `.quick-open-empty` padding | 8px 14px | 6px 12px |

Keep: overlay width `min(640px, 86vw)`, `top: 4px`, list `max-height: 360px`, active row `#37373d`, chips 16×16, type colors, placeholder `Search files by name`.
...
- Served CSS includes `#quick-open-backdrop` with `position: fixed` and `inset: 0` (or equivalent `top/right/bottom/left: 0`), and `.quick-open-item` `min-height: 24px`.
...
6. Rows are visually tighter; active `#37373d` and match `#4fc1ff` unchanged.
...
4. Overlay input and rows use the specified denser spacing.
...
| `internal/ui/style.css` | Backdrop rules; denser input/row/empty padding; delete footer rules |
